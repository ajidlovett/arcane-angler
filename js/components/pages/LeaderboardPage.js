        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {leaderboardData.map((playerRow, index) => {
                  const rank = index + 1;
                  return (
                    <tr
                      key={playerRow.user_id}
                      className={`border-t border-${theme.border} ${
                        playerRow.user_id === user?.id ? `bg-${theme.hover} bg-opacity-50` : `hover:bg-${theme.primarySolid}`
                      }`}
                    >
                      <td className={`px-2 sm:px-4 py-3 font-bold text-xs sm:text-sm ${getRankColor(rank)}`}>
                        {getRankIcon(rank)}
                      </td>
                      <td className="px-2 sm:px-4 py-3 text-xs sm:text-sm">
                        <div className="flex items-center gap-2">
                          <span className="text-white font-semibold">
                            {getCountryFlag(playerRow.nationality)} {playerRow.profile_username}
                          </span>
                          {playerRow.equipped_title && getTitleName(playerRow.equipped_title) && (
                            <span className="text-yellow-400 text-xs">
                              - {getTitleName(playerRow.equipped_title)}
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="px-2 sm:px-4 py-3 text-right text-white font-bold text-xs sm:text-sm">
                        {formatNumber(getValueForCategory(playerRow, selectedCategory))}
                      </td>
                    </tr>
                  );
                })}
