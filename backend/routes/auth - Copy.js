cat > /home/ajid/public_html/backend/routes/auth.js << 'ENDOFFILE'
...
router.post('/register', async (req, res) => {
    try {
        ...
        // Generate JWT token (allow login even without verification for better UX)
...
router.get('/verify-email/:token', async (req, res) => {
    try {
        ...
        res.json({
            message: 'Email verified successfully! You can now enjoy all features.',
            user: {
                id: user.id,
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        // Find user
        const [users] = await db.query(
            'SELECT id, username, profile_username, email FROM users WHERE email = ?',
            [email]
        );
...
router.get('/verify', async (req, res) => {
    try {
        ...
        jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
            ...
            const [users] = await db.query(
                'SELECT id, username, profile_username, email, email_verified FROM users WHERE id = ?',
                [decoded.userId]
            );
